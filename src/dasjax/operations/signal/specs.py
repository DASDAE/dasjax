...
OPERATIONS: tuple[OperationSpec, ...] = (
    OperationSpec(
        ...
        test_cases=tuple(
            OperationCase(
                ...
                resolve_call=lambda patch, kind=kind: ((), {"dim": patch.dims[-1], "type": kind}),
                baseline=lambda patch, args, kwargs: baseline_patch_method(patch, args, kwargs, "detrend"),
            )
    ...
    OperationSpec(
        ...
        test_cases=tuple(
            OperationCase(
                ...
                resolve_call=lambda patch, norm=norm: ((), {"dim": patch.dims[-1], "norm": norm}),
                baseline=lambda patch, args, kwargs: baseline_patch_method(patch, args, kwargs, "normalize"),
            )
    ...
    OperationSpec(
        ...
        test_cases=(
            OperationCase(
                ...
                baseline=lambda patch, args, kwargs: dc_differentiate.func(patch, *args, **kwargs),
            ),
            OperationCase(
    ...
    OperationSpec(
        ...
        test_cases=(
            OperationCase(
                ...
                resolve_call=lambda patch: ((), {"dim": patch.dims[-1]}),
                baseline=lambda patch, args, kwargs: dc_integrate.func(patch, *args, **kwargs),
            ),
            OperationCase(
                case_id="integrate-definite",
                resolve_call=lambda patch: ((), {"dim": patch.dims[-1], "definite": True}),
                baseline=lambda patch, args, kwargs: dc_integrate.func(patch, *args, **kwargs),
            ),
        ),
    ),
    OperationSpec(
        ...
        test_cases=(
            OperationCase(
                case_id="taper-time",
                resolve_call=lambda patch: ((), {"window_type": "hann", patch.dims[-1]: 0.05}),
                baseline=lambda patch, args, kwargs: dc_taper.func(patch, *args, **kwargs),
            ),
        ),
    ),
    OperationSpec(
        ...
        test_cases=(
            OperationCase(
                case_id="taper-range",
                resolve_call=lambda patch: ((), {"window_type": "hann", patch.dims[-1]: (10, 20), "samples": True}),
                baseline=lambda patch, args, kwargs: dc_taper_range.func(patch, *args, **kwargs),
            ),
        ),
    ),
)
