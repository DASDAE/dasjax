...
OPERATIONS: tuple[OperationSpec, ...] = (
            ...
            OperationCase(
                ...
                resolve_call=lambda patch: (
                    (),
                    {
                        "mode": "constant",
                        "samples": True,
                        get_preferred_transform_dim(patch): (2, 3),
                    },
                ),
                prepare_patch=prepare_even_patch_for_dim,
                baseline=lambda patch, args, kwargs: impl.pad_patch(
                    patch, *args, **kwargs
                ),
            ),
            OperationCase(
                ...
                resolve_call=lambda patch: (
                    (),
                    {get_preferred_transform_dim(patch): "fft"},
                ),
                baseline=lambda patch, args, kwargs: dc_pad.func(
                    patch, *args, **kwargs
                ),
            ),
        ),
    ...
    OperationSpec(
        ...
        patch_op_cls=impl.DftOp,
        test_cases=(
            OperationCase(
                case_id="dft-real",
                resolve_call=lambda patch: ((), {"dim": patch.dims[-1], "real": True}),
                baseline=lambda patch, args, kwargs: dc_dft.func(
                    patch, *args, **kwargs
                ),
            ),
            OperationCase(
                ...
                resolve_call=lambda patch: (
                    (),
                    {"dim": patch.dims[-1], "real": None, "pad": False},
                ),
                baseline=lambda patch, args, kwargs: dc_dft.func(
                    patch, *args, **kwargs
                ),
    ...
    OperationSpec(
        ...
        patch_op_cls=impl.IdftOp,
        test_cases=(
            OperationCase(
                case_id="idft-real",
                resolve_call=lambda patch: ((), {}),
                prepare_patch=prepare_fourier_patch,
                baseline=lambda patch, args, kwargs: dc_idft.func(
                    patch, *args, **kwargs
                ),
            ),
        ),
    ...
    OperationSpec(
        ...
        patch_op_cls=impl.HilbertOp,
        test_cases=(
            OperationCase(
                case_id="hilbert-time",
                resolve_call=lambda patch: resolve_even_dim_call(patch),
                baseline=lambda patch, args, kwargs: dc_hilbert.func(
                    patch, *args, **kwargs
                ),
            ),
        ),
    ...
    OperationSpec(
        ...
        patch_op_cls=impl.EnvelopeOp,
        test_cases=(
            OperationCase(
                case_id="envelope-time",
                resolve_call=lambda patch: resolve_even_dim_call(patch),
                baseline=lambda patch, args, kwargs: dc_envelope.func(
                    patch, *args, **kwargs
                ),
            ),
        ),
    ...
    OperationSpec(
        ...
        patch_op_cls=impl.WhitenOp,
        test_cases=(
            OperationCase(
                case_id="whiten-default",
                resolve_call=lambda patch: (
                    (),
                    {get_preferred_transform_dim(patch): None},
                ),
                baseline=lambda patch, args, kwargs: dc_whiten.func(
                    patch, *args, **kwargs
                ),
            ),
            OperationCase(
                ...
                resolve_call=lambda patch: (
                    ...
                    {
                        "smooth_size": 3.0,
                        get_preferred_transform_dim(patch): (5.0, 10.0, 20.0, 25.0),
                    },
                ),
                baseline=lambda patch, args, kwargs: dc_whiten.func(
                    patch, *args, **kwargs
