...
The package currently registers 75 pipeline operations. Most are implemented as JAX-backed kernels with static metadata planning; heavier DASCore-compatible numeric transforms can use host callbacks when a full static JAX kernel is not yet practical. Callback-backed operations are compatibility paths and generally offer less fusion benefit than native kernels.
